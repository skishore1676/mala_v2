...
| P2 | Metric library ("fires where he fired") | Claude ~1 week | **IN PROGRESS 2026-07-11 — 2 of 4 playbooks PASS** (`scripts/p2_detector_scorecard.py`, IWM/SPY sweep, 73.8k bar-dir evals): **FLASH PASS** F-C 0.15ATR/≤15m: 59% HIGH recall, 7.4× lift, 4 fires/day (F-A near-miss: 96% recall, 6.3×, 6/day); **TREND PASS** T-C active-stalling-pullback: 50% recall (at bar), 8.7× lift, 4/day; **EXHAUSTION fork (b)** — run-context = recall 97% but lift 1.9× (a run always exists to fade), trigger-as-coded (stall+retest-gap at entry) = 17% recall because operator enters AFTER the failed retest + consolidation break; next: event-based failed-retest-in-lookback vs multi-day reference extremes; **RANGE unmeasurable on IWM/SPY** (0 gold entries — semis/single-name play), defers to hypothesis-driven P3. Caveat: recall vs machine-tagged gold is partially circular for detectors that mirror the tagging rules; lift/fires-day are independent | see §P2 |
