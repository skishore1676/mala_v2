...
| P3 | Mechanical validation (option-path, native exits) | Claude ~1 week | **DONE 2026-07-11 — no unconditional pass; terrain map delivered** (`scripts/p3_option_path_backtest.py` + `p3_selection_rerun.py`; results in `data/personal_imports/tagged/p3_*`): detector fires on IWM/SPY 2021→2026 carry a REAL but THIN gross edge (+0.5..+3.4%/trade on premium, 44/48 regime cells positive, native profile exits, IV-banded) that does NOT survive costs (4% conservative / 2% realistic haircut) under any live-translatable selection: all-fires fail; hindsight best-of-day DOUBLES gross to +3-4% (proves edge concentrates in strong fires) but is lookahead; first-of-day and absolute-strength-threshold (fit 2021-22, OOS after) recover only part — single marginal survivor IWM T-C long +0.5% net@2%, 2/3 regimes, too thin alone. **Named finding: the "selection gap" ≈ 2-4%/trade — the operator's discretionary choice of WHICH ~1-2 of ~12 daily fires to take is the unmechanized layer.** Yardstick was right this time (no M1 repeat): fork map, not kill | see §P3 + diary 2026-07-11 |
| P4 | Promotion to shadow lanes | existing funnel | **REFRAMED — operator decision gate**: no unconditional mechanical candidate to promote; recommended path = deploy FLASH/EXH detectors as CONSULTATION signals (alerts / morning-bias inputs, fits `docs/PLAYBOOK_CONSULTATION_LAYER.md`) so operator discretion supplies the selection layer AND every take/skip becomes labeled data to learn the selection function; alternatives: (b) model P(operator takes fire) from 47.8k annotated fires × his 1,465 entries, (c) execution/cost attack (edge is cost-sized), (d) single-name/semis habitat tests (corpus was IWM/SPY only) | see §P4 |
