...
if [[ "$SKIP_DRY_START" != "1" ]]; then
  (
    ...
    ./.venv/bin/python -m bhiksha.tools.trade_session \
      ...
      --live \
      --max-bars 0
  ) > "$SNAP_ROOT/bhiksha/dry_start_after.txt" 2>&1
  sed -E \
    -e 's/account_id=[^ ]+/account_id=<redacted>/g' \
    -e 's/client_id=[^& ]+/client_id=<redacted>/g' \
    -e 's/code=[^& ]+/code=<redacted>/g' \
    -e 's/session=[^& ]+/session=<redacted>/g' \
    "$SNAP_ROOT/bhiksha/dry_start_after.txt" \
    | grep -E '^(HEALTH|TOKEN_HEALTH|SYNC positions=|Stopping after warm start)' || true
  echo "DRY_START_OK log=$SNAP_ROOT/bhiksha/dry_start_after.txt"
fi
