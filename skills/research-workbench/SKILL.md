...
./.venv/bin/python -m src.research.research_ops digest --days 1 --with-control --with-intake
...
./.venv/bin/python -m src.research.research_ops push-intake-template --intake-sheet-name Research_Intake
./.venv/bin/python -m src.research.research_ops process-intake --apply
...
- `push-intake-template` and `process-intake` power the `Research_Intake` tab for new hypotheses. Intake evaluates current-code feasibility before creating any hypothesis file.
- `digest` writes the daily/weekly operating brief for humans and advisory agents.
...
./.venv/bin/python -m src.research.local_orchestrator once --mode apply-safe --with-control-sheet
...
With `--with-control-sheet`, the orchestrator reads `Research_Control` and may
execute only explicit operator actions: `APPROVE_RETUNE`, `APPROVE_PUBLISH`,
`APPROVE_BOARD_SYNC`, `MARK_STALE`, or `SKIP`.
