...
def _render_report(
    *,
    ...
    lines = [
        "# Bhiksha Signal Concordance and Realized Signal Expected Value",
        "",
        ...
        "## Plain-English Read",
        "",
        "- Signal Expected Value asks: when Bhiksha fired a Mala-sourced signal, did it match Mala's expected signal, did it become a trade plan, and did the resulting option trade make or lose money versus the expected edge?",
        "- Treat this as the strategy-quality and concordance report. It is different from the daily shadow report, which mainly checks whether the session produced observations, blocks, and runtime issues.",
        "",
        "## Metric Glossary",
        "",
        "- `true_signal_events`: strategy conditions that fired in Bhiksha for active deployments.",
        "- `trade_plans`: true signals that reached option preview / entry-planning. A plan can still be blocked by budget, lifecycle state, spread, open interest, or other execution gates.",
        "- `matched_signal_to_entry`: trade plans linked back to a prior true signal from the same deployment.",
        "- `closed_trades_with_realized_ev`: closed option trades where the report could compute realized PnL and stop-R.",
        "- `positive_trades_vs_evidence` / `adverse_trades_vs_evidence`: closed trades whose realized option result helped or hurt the Mala evidence thesis.",
        "- `same_bar_match_rate`: share of Bhiksha true signals that a same-bar Mala replay also sees as valid. Low match rate is plumbing/provider/adoption pressure before it is alpha evidence.",
        "",
        "## Method",
        "",
